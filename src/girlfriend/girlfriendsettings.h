...
enum class MoodInfluenceLevel { Low, Medium, High };
...
public:
    ...
    // Avatar Level
    AvatarLevel avatarLevel() const {
        return m_avatarLevel;
    }
    void setAvatarLevel(AvatarLevel level);
    QString avatarLevelPath() const;

    // Mood Influence
    MoodInfluenceLevel moodInfluence() const {
        return m_moodInfluence;
    }
    void setMoodInfluence(MoodInfluenceLevel level);

    // Video Sound (only for Level 3)
    bool videoSoundEnabled() const {
        return m_videoSoundEnabled;
    }
    void setVideoSoundEnabled(bool enabled);

    // Voice Output
    bool voiceOutputEnabled() const {
        return m_voiceOutputEnabled;
    }
    ...
    QString xfyunAppId() const {
        return m_xfyunAppId;
    }
    void setXfyunAppId(const QString& id);

    QString xfyunApiKey() const {
        return m_xfyunApiKey;
    }
    void setXfyunApiKey(const QString& key);

    QString xfyunApiSecret() const {
        return m_xfyunApiSecret;
    }
    void setXfyunApiSecret(const QString& secret);

    QString xfyunAsrUrl() const {
        return m_xfyunAsrUrl;
    }
    void setXfyunAsrUrl(const QString& url);

    QString xfyunTtsUrl() const {
        return m_xfyunTtsUrl;
    }
    void setXfyunTtsUrl(const QString& url);

    QString xfyunVoiceType() const {
        return m_xfyunVoiceType;
    }
