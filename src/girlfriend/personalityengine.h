...
public:
    ...
    // Affection parsing result
    struct AffectionResult {
        double change;      // mood change value (-0.3 to +0.3)
        bool hasTag;        // true if AI provided affection tag, false if fallback used
        QString cleanText;  // text after removing affection tag
    };
    // Parses affection tag from AI response, falls back to keyword detection if no tag
    AffectionResult parseAffectionFromResponse(const QString& text, const QString& userInput) const;

    // Helper: keyword-based mood detection (for fallback)
    double detectMoodChangeFromKeywords(const QString& userInput) const;

    // Update mood based on AI evaluation or fallback result
    void updateMoodFromAI(double change);
