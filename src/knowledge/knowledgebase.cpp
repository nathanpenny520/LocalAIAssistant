...
KnowledgeBase* KnowledgeBase::instance() {
    if (!s_instance) s_instance = new KnowledgeBase();
    return s_instance;
}

KnowledgeBase::KnowledgeBase(QObject* parent)
        : QObject(parent), m_embedder(new Embedder()), m_vectorDB(new VectorDB()) {
}
...
bool KnowledgeBase::importDocument(const QString& filePath) {
    if (!m_ready || !m_importer) return false;
...
bool KnowledgeBase::removeDocument(const QString& filePath) {
    if (!m_ready || !m_vectorDB) return false;
...
bool KnowledgeBase::isKnowledgeQuery(const QString& message) {
    ...
    if (instance()->totalDocuments() == 0) return false;

    static const QStringList queryKeywords = {
            // Chinese — explicit knowledge base references
            QStringLiteral("知识库"),
            QStringLiteral("导入的文档"),
            QStringLiteral("我的文档"),
            QStringLiteral("文档里"),
            QStringLiteral("我导入"),
            QStringLiteral("之前上传"),
            // Chinese — search / look up in documents
            QStringLiteral("查一下文档"),
            QStringLiteral("搜一下"),
            QStringLiteral("搜索文档"),
            QStringLiteral("找一下文档"),
            QStringLiteral("有没有关于"),
            QStringLiteral("资料里"),
            ...
            QStringLiteral("search my files"),
            QStringLiteral("look up in"),
            QStringLiteral("based on the document"),
            QStringLiteral("imported doc"),
    };
