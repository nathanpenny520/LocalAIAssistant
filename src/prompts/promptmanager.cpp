    ...
    if (prompt.isEmpty()) {
        ...
        if (lang == QStringLiteral("en")) {
            prompt = QStringLiteral(
                    ...
                    "for casual chat)\n\n"
                    "## Iteration Loop\n\n"
                    "When you receive [ITERATION_FEEDBACK], parse the JSON object after the tag "
                    "for structured execution results. The JSON contains:\n"
                    "- \"iteration\": loop iteration number\n"
                    "- \"summary\": {\"total\", \"succeeded\", \"failed\"}\n"
                    "- \"operations\": array of {index, type, description, success, exit_code, "
                    "stdout, stderr, error, elapsed_ms}\n\n"
                    "If summary.failed == 0 and task is done -> output [TASK_COMPLETE] with a "
                    "summary\n"
                    "If more steps needed -> output new [TASK_PLAN]\n"
                    "If something failed -> analyze stderr/error, adjust, output new "
                    "[TASK_PLAN]\n"
            ...
            prompt = QStringLiteral(
                    ...
                    "## 迭代循环\n\n"
                    "收到 [ITERATION_FEEDBACK] 后，解析标签后的 JSON 对象获取结构化执行结果。"
                    "JSON 包含：\n"
                    "- \"iteration\": 迭代次数\n"
                    "- \"summary\": {\"total\", \"succeeded\", \"failed\"}\n"
                    "- \"operations\": 数组，每项包含 {index, type, description, success, "
                    "exit_code, stdout, stderr, error, elapsed_ms}\n\n"
                    "若 summary.failed == 0 且任务完成 → 输出 [TASK_COMPLETE] 并附上总结\n"
                    "若需要更多步骤 → 输出新的 [TASK_PLAN]\n"
                    "若某步骤失败 → 分析 stderr/error，调整后输出新的 [TASK_PLAN]\n"
