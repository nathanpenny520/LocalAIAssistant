    ...
    , m_streamingCheckBox(new QCheckBox(tr("启用流式输出"), this))
{
    ...
    auto updateKbStatus = [this, kb, kbStatusLabel]() {
        ...
        if (docs == 0) {
            ...
            kbStatusLabel->setText(SettingsDialog::tr("状态：%1 个文档，%2 个片段").arg(docs).arg(chunks));
        }
        ...
        m_kbDocList->clear();
        for (const QString &doc : kb->allDocuments())
            m_kbDocList->addItem(doc);
    };
    updateKbStatus();
    ...
    connect(m_kbDocList, &QListWidget::itemSelectionChanged, this,
            [deleteBtn, this]() {
    ...
    connect(importBtn, &QPushButton::clicked, this, [this, kb, updateKbStatus, importBtn]() {
        QStringList files = QFileDialog::getOpenFileNames(
            this, tr("选择要导入的文档"), QString(),
            tr("文档文件 (*.txt *.md *.pdf *.docx);;所有文件 (*)"));
        if (files.isEmpty())
            return;
        ...
        QTimer::singleShot(3000, importBtn, [importBtn, origText]() {
            importBtn->setText(origText);
        });
    });
    ...
    QLabel *confirmDesc = new QLabel(
        tr("关闭后，AI 将直接执行文件操作而不弹出确认对话框。建议保持开启。"), this);
    ...
    connect(okButton, &QPushButton::clicked, this, [this]() {
        ...
        // Security settings
        QStringList whitelist = m_pathWhitelistEdit->toPlainText().split(
        ...
        StyleSheetManager::Theme theme = static_cast<StyleSheetManager::Theme>(
            m_themeComboBox->currentData().toInt());
...
{
    return m_pathWhitelistEdit->toPlainText().split(
        QChar::LineFeed, Qt::SkipEmptyParts);
}
